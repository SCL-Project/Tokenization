		...
		const initWeb3 = async () => {
			if (window.ethereum) {
				try {
					...
					const tempStableCoinContract = new web3.eth.Contract(StableCoinABI, stableCoinAddress)
	...
	const approveCredit = async (borrowerAddress, amount, period) => {
		...
		try {
			...
			await stableCoinContract.methods
				.approve(p2pLendingContractAddress, amountInWei)
		...
		console.log(
			`Navigating with borrowerAddress: ${borrowerAddress}, amount: ${amount}, period: ${period}`
		)
		...
		<Box display='flex' flexDirection='column' alignItems='center' mt={2}>
			...
			{requests.map((request, index) => (
				<Paper
					key={index}
					elevation={3}
					style={{ margin: '16px 0', padding: '16px', width: '95%', backgroundColor: '#F5F5F5' }}>
					<Grid container spacing={2}>
						...
						<Grid item xs={12} sm={4}>
							<Typography variant='body1'>
								<strong>Period:</strong> {request.period} days
							</Typography>
						</Grid>
						<Grid item xs={12} sm={4}>
							<Typography variant='body1'>
						...
						<Grid item xs={12}>
							<Button
								...
								color='primary'
								onClick={() =>
									onApproveCredit(request.borrowerAddress, request.amount, request.period)
								}
								sx={{
									backgroundColor: '#7393B3', // Button background color
									color: '#ffffff', // Text color

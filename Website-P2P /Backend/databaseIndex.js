...
app.post('/register', (req, res) => {
	// Load the variables
	const { firstname, lastname, address, city, plz, ahv_number, email, password } = req.body
	...
	db.query('SELECT email FROM users WHERE email = ?', [email], (error, results) => {
		...
		bcrypt.hash(password, 10, (err, hash) => {
			...
			const sql =
				'INSERT INTO users (firstname, lastname, address, city, plz, ahv_number, email, password) VALUES (?, ?, ?, ?, ?, ?, ?, ?)'

			// Execute the SQL to insert the user data into the database
			db.query(sql, [firstname, lastname, address, city, plz, ahv_number, email, hash], error => {
				if (error) {
